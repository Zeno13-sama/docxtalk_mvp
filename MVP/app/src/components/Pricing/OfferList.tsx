...
const OfferList = ({ text, status }: { text: string; status: "active" | "inactive" }) => {
  ...
  const iconClasses = `mr-3 flex h-[18px] w-full max-w-[18px] items-center justify-center rounded-full ${
    theme === 'dark' ? 'bg-sky-500 text-white' : 'bg-primary bg-sky-400 text-primary'
  }`;

  const textClasses = `m-0 text-base font-medium ${
    theme === 'dark' ? 'text-white' : 'text-body-color'
  }`;
